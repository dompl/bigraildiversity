...
if ( $calls ):
                ...
                foreach ( $calls as $call ):
                    ...
                    if ( $call['img'] ) {
                        ...
                        $image = wpimage( 'img=' . (int) $call['img'] . '&h=' . 100 . '&w=' . 100 . '&retina=false&upscale=true&crop=true&single=false' );
...
printf( '<div class="call-image"><figure><img src="%s" alt="%s" height="' . $img_height . '"></figure></div>',
	...
	<script>
	jQuery(function() {
		jQuery('.custom-call-title-<?php echo $random ?>').fitText(1.5, {
			minFontSize: '<?php echo (int) round(  ( str_replace( 'px', '', $font_size ) / 1.5 ), 0 ) . 'px' ?>',
			maxFontSize: '<?php echo $font_size ?>'
		});;
	});
	</script>
	<?php endif;?>
