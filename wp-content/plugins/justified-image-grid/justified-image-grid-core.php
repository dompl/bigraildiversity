...
filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#f8f8f8', endColorstr='#eeeeee',GradientType=0 );",
											...
											'scripts_to_load'			=> array('prettyphoto','colorbox','magnific','photoswipe4','pixastic','dotdotdot'),
											'jquery'					=> 'nochange',
...
text-align:left;",
			...
			if(!$case){
				...
				if(!is_admin() &&
					(isset($GLOBALS['pagenow']) === false || $GLOBALS['pagenow'] !== 'wp-login.php')
				){
					...
					if(!empty($provider) && !empty($content_id)){
						...
						switch ($provider) {
							...
							case 'FB':
								...
								$photos_url = 'https://graph.facebook.com/v3.1/'.$content_id.'?fields=name,images&access_token='.$token;
									...
									if(!empty($this->settings['fb_authed'])){
										foreach($this->settings['fb_authed'] as $authed_entity){
											if(!empty($authed_entity['access_token']) &&
												$authed_entity['access_token'] !== 'public' &&
												strpos($authed_entity['access_token'], '|') === false
											){
		...
		function jig_jquery_override_and_unconditional(){
			...
			if ($this->settings['jquery'] != 'nochange') {
				switch($this->settings['jquery']){
					...
					case 'google3plugin':
						...
						$fallback_url['google3plugin'] = plugins_url('js/jquery-3.3.1.min.js', __FILE__);
						...
						$fallback_version['google3plugin'] = '3.3.1';
						...
						if($this->settings['jquery'] == 'google2wp' || $this->settings['jquery'] == 'google2plugin'){
							...
							$url = $protocol . '://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js';
						}else{
					...
					case 'plugin3':
						wp_deregister_script('jquery');
						$url = plugins_url('js/jquery-3.3.1.min.js', __FILE__);
						wp_register_script('jquery', $url, array(), '3.3.1', $footer);
						wp_register_script('jquery-migrate-jig', plugins_url('js/jquery-migrate-3.0.0.min.js', __FILE__), array(), '3.0.0', $footer);
			...
			if($this->settings['conditional_script_loading'] == 'no'){
				...
				if(in_array('dotdotdot', $this->settings['scripts_to_load'])){
					wp_enqueue_script('dotdotdot', plugins_url('js/jquery.dotdotdot.js', __FILE__), 'jquery', '1.8.3', true);			
				}
		...
		function jig_build_settings_page(){
			...
			<script type="text/javascript">
				...
				(function($){
						...
						if(!data['error']){
							...
							if(existing.length == 0){
								...
								$('#jigFbAuthedHidden').append(function(){
									...
									jig_message_box(
										'<?php _e("The authentication of", "jig_td"); ?> '
										+page_types[data['type']]
										+': '
										+data['user_name']
										+' <?php _e("is successful. You can choose from", "jig_td"); ?> '
										+data['info']['album_count']
										+'.'
										+(data['info']['page_count'] ? ' '+data['info']['page_count'] : '')
										+(data['info']['expires'] ?
											(data['access_token_owner_name'] &&
												data['type'] == 'page' ?
												' <?php _e("It has access from the", "jig_td"); ?> '+page_types[data['info']['owner_type']]+': '+data['access_token_owner_name']+'.' 
												: '')
											+' <?php _e("It will expire in", "jig_td"); ?> '
											+data['info']['time_remaining']
											+' <?php _e("which is on", "jig_td"); ?> '
											+jig_expires_date(
												parseFloat(data['info']['time_added']) + 
												parseFloat(data['info']['expires'])
											)
											+'.'
										: '')
									);
									jig_fb_track_down_owned_pages(data);
								...
								jig_message_box('<?php _e("The re-authentication of already existing", "jig_td"); ?> '+
										...
										data['info']['album_count']+'. '+data['info']['page_count']+
										(data['info']['expires']
					...
					function jig_fb_track_down_owned_pages(data){
						// Only for profiles that own pages
						if(data['info']['pages']){
							var pageCount = data['info']['pages'].length;
							jig_fb_ajax_loading('show');
							window["jig_doing_ajax"] = 0;
							for(var i = 0; i < pageCount; i++){
								window["jig_doing_ajax"]++;
								if($('#fbAuthed'+data['info']['pages'][i]['id']).length === 0){
									$.post(
										ajaxurl,
										{
											'action': 'jig_add_fb_page',
											'security': '<?php echo wp_create_nonce("jig_add_fb_page") ?>',
											'page': data['info']['pages'][i]['id'],
											'token': data['info']['pages'][i]['token'],
											'owner': {
												'user_name': data['user_name'],
												'user_id': data['user_id'],
												'owner_type' : data['type'],
												'expires': data['info']['expires'], 
												'time_added': data['info']['time_added'],
												'time_remaining': data['info']['time_remaining']
											}
										},
										function(data) {
											jig_fb_ajax_process(data);
											window["jig_doing_ajax"]--;
											if(window["jig_doing_ajax"] === 0){
												jig_fb_ajax_loading('hide');
											}
										},
										'json'
					...
					function jig_import_settings(){
						var jigSettingsImportText 	= $('#jigSettingsImportText'),
							encodedSettings 		= jigSettingsImportText.val();
						...
						ajaxurl,
						{
							...
							'encoded_settings' : encodedSettings
						},
						function(data) {
							if(!data['error']){
...
jQuery('#jigPresetSelect').val('c".$custom_preset_id."');});</script>";
			...
			add_settings_field(
				...
				array(	'id' => 'jquery',
						...
						'inputs' => array(
							...
							'google3plugin' => __("jQuery v3.3.1 from Google, fallback to JIG's (experimental!).", 'jig_td'),
							'plugin' => __('jQuery v1.8.3 bundled in JIG.', 'jig_td'),
							'plugin2' => __('jQuery v2.2.4 bundled in JIG.', 'jig_td'),
							'plugin3' => __('jQuery v3.3.1 + migrate, bundled in JIG (experimental!).', 'jig_td'),
				...
				array(	'id' => 'lightbox',
						...
						'inputs' => array(
							...
							'socialgallery' => $this->social_gallery_plugin_data[0] === false ?
								__('Social Gallery (not active!) is purchased separately', 'jig_td').', <a href="https://codecanyon.net/item/social-gallery-wordpress-photo-viewer-plugin/2665332?ref=Belcovio" target="_blank">'.__("here", 'jig_td').'</a>.'
								: 'Social Gallery',
		...
		function jig_print_rml_desc(){
			...
			if(!class_exists('RML_Core') && !class_exists('MatthiasWeb\RealMediaLibrary\general\Core')){	
				echo '<span class ="jig_source_plugin_not_istalled">'.sprintf(__('WP Real Media Library is not installed! Purchase your copy on %s', 'jig_td'),'<a href="https://goo.gl/5lcIq4" target="_blank">CodeCanyon</a>').'.</span>';  
		...
		function jig_print_facebook_desc(){
			echo '<div id="jig_facebook" class="jigLong"><p>'.
				...
				'<li>'.__('Click "Add a New App", then enter a "Display Name" and hit "Create App ID" (solve the captcha).', 'jig_td').'</li>'.
				...
				'</ol></ul><li>'
					...
					.'</li><li>'
					.__('<strong>Do NOT get it reviewed</strong> - when you add your current user just ignore "Submit for Login Review - Some of the permissions below have not been approved for use by Facebook." as it does not apply to you, since you are the admin.', 'jig_td').'</li></ul></div><p>'.
		...
		function jig_print_flickr_desc(){
			echo '<div id="jig_flickr" class="jigLong"><p>'.
				...
				'<li>'.sprintf(__('Log in to Flickr, then go to %1$s.', 'jig_td'), '<a href="https://www.flickr.com/services/apps/create/apply/" target="_blank">Flickr Services</a>').'</li>'.
		...
		function jig_print_fb_authed(){
			...
			$output .= '<div id="jigFbAuthOtherUserPanel">
							<ol>
								...
								<li>'.__('Send him/her this link', 'jig_td').':<br /><input type="text" id="jigFbOtherUserLink" value="" data-force="https://www.facebook.com/v3.1/dialog/oauth?client_id='.trim($this->settings['fb_app_id']).'&scope=user_photos,user_posts,user_videos,pages_show_list' . '&redirect_uri='.urlencode(plugins_url('fb-auth-other-user.php', __FILE__)).'" /></li>
		...
		*/
		...
		function jig_take_over_rml_shortcode($output = '', $attr, $instance = false) {
		...
		function remove_nextgen_resources() {
			...
			foreach ($wp_scripts->registered as $handle => $registered_script) {
				if(strpos($registered_script->src, '/nextgen-gallery/') !== false ||
					strpos($registered_script->src, '/nextcellent-gallery-nextgen-legacy/') !== false
				){
			...
			foreach ($wp_styles->registered as $handle => $registered_style) {
				if(strpos($registered_style->src, '/nextgen-gallery/') !== false ||
					strpos($registered_style->src, '/nextcellent-gallery-nextgen-legacy/') !== false
				){
		...
		function jig_add_xml_sitemap_images($images, $post_id){
			...
			if(!empty($jig_images_for_xml_sitemap)){
				...
				$return_images_for_sitemap = array();
	        	...
	        	if(!empty($images)){
		        	foreach ($images as $images_key => $images_val) {
						...
						if($last_dot_pos_in_images_val_src !== false){
		...
		function jig_init_shortcode_for_xml_sitemap($atts = array()){
			if(!is_array($atts)){
				$atts = array();
			}
...
*/
				...
				case 'nextgen':
					...
					if(!isset($jigNgConnect)){
						...
						if($this->jig_ng_get_query_var('gallery') !== ''){
							...
							$ng_album = '';
							$jigNgConnect = true;
						}elseif($this->jig_ng_get_query_var('album') !== ''){
						...
						}elseif($this->jig_ng_get_query_var('gallerytag') !== ''){
					...
					}elseif($ng_album !== ''){ // If an album (or overview album) is displayed
				...
				case 'facebook':
					...
					if($facebook_album == 'overview' || $facebook_album == 'overview_only_albums'){
						if($fb_lightbox_album == 'yes'){
							if(empty($facebook_album_multi) || (!empty($facebook_album_multi) && $fb_album_exclude == 'yes')){ // Preventing showing everything at once with lightbox albums.
								$preliminary_count_check_albums_url = 'https://graph.facebook.com/v3.1/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(count)&access_token='.$token;
							...
							$albums_url = 'https://graph.facebook.com/v3.1/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(id,cover_photo,link,count,name,description,type,photos.limit('.$auto_limit.').fields(images,source,height,width,name))&access_token='.$token;
						}else{
							$albums_url = 'https://graph.facebook.com/v3.1/'.$facebook_id.'?fields=albums.limit('.$limit.').fields(id,cover_photo,link,count,name,description,type,photos.limit(1).fields(images))&access_token='.$token;
						}
						...
						if(!empty($facebook_album_multi) && $facebook_album_multi[0] == 'latestone'){
							...
							$albums_url = 'https://graph.facebook.com/v3.1/'.$facebook_id.'?fields=albums.limit(5).fields(id,cover_photo,link,count,name,description,type,photos.limit('.$limit.').fields(images,source,height,width,name))&access_token='.$token;
									...
									if(!empty($album->count) && !empty($album->link) && !empty($album->photos->data)){
										if($fb_lightbox_album == 'no' && $fb_actual_cover_photo == 'yes' && !empty($album->cover_photo)){
											...
											if(is_string($album->cover_photo)){
												$fb_cover_photo_url = 'https://graph.facebook.com/v3.1/'.$album->cover_photo.'?fields=images&access_token='.$token;
											}elseif(!empty($album->cover_photo->id)){
												$fb_cover_photo_url = 'https://graph.facebook.com/v3.1/'.$album->cover_photo->id.'?fields=images&access_token='.$token;
											}
										...
										if(!empty($album->photos)){
											...
											}elseif(($facebook_description == 'yes' || $facebook_description == 'thumbnails') && !empty($album->description)){
											...
											if($fb_lightbox_album == 'yes'){ // If gallery should be displayed as a lightbox
												...
												foreach ($subalbum->data as $subalbum_image) {
													...
													}elseif($facebook_image_size == 'maximum'){
														$shadow_data['url'] = $subalbum_image->images[0]->source;
														if($lightbox == 'photoswipe'){
															$shadow_size = ' data-wh="'.$subalbum_image->images[0]->width.'x'.$subalbum_image->images[0]->height.'"';
														}
					...
					if($facebook_album == 'feed'){
						...
						$feed_url = 'https://graph.facebook.com/v3.1/'.$facebook_id.'/feed?fields=picture,caption,description,message,object_id'.$limit_parameter.'&access_token='.$token;
						...
						if(!empty($photos)){
							...
							foreach ($photos as &$feed_post) {
								if(!(isset($feed_post->picture) && isset($feed_post->object_id))){
									...
									$photo_object_url = 'https://graph.facebook.com/v3.1/'.$feed_post->object_id.'?fields=images,source,height,width&access_token='.$token;
					...
					if($facebook_album == 'videos'){
						$video_url = 'https://graph.facebook.com/v3.1/'.$facebook_id.'/videos/uploaded?fields=source,format,title,description'.$limit_parameter.'&access_token='.$token;
					...
					if($facebook_album !== 'overview' && $facebook_album !== 'overview_only_albums' && $facebook_album !== 'feed' && $facebook_album !== 'videos'){
						...
						$photos_url = "https://graph.facebook.com/v3.1/".$facebook_album."/photos?fields=source,height,width,name".($facebook_image_size == 'normal' ? '' : ',images').$limit_parameter.'&access_token='.$token;
						...
						if(!empty($photos) && empty($photos['message'])){
							...
							if(($facebook_description == 'yes' || $facebook_description == 'above') && empty($facebook_description_displayed_already)){
								$facebook_album_url = "https://graph.facebook.com/v3.1/".$facebook_album."?fields=description".'&access_token='.$token;
						...
						if($flickr_photoset === '' && ((!empty($collections[$flickr_collection]['set']) && count($collections[$flickr_collection]['set']) > 0) || (!empty($collections[$flickr_collection]['collection']) && count($collections[$flickr_collection]['collection']) > 0))){
							...
							if(!empty($collections[$flickr_collection]['set']) && count($collections[$flickr_collection]['set']) > 0){
								...
								foreach ($collections[$flickr_collection]['collection'] as $single_collection) {
									$data['url'] = (substr($single_collection['iconlarge'],0,1) !== '/' ? $single_collection['iconlarge'] : 'https://www.flickr.com/images/collection_default_l.gif');
									$data['width'] = 179;
									$data['height'] = 134;
			...
			if($caption != 'off'){
				...
				if($caption_match_width == 'no' && $caption !==  'below'){
					...
					}elseif($caption_bg_color == 'transparent' || $caption_bg_color == 'none') {
						switch ($caption_align) {
							case 'center':
								$caption_appearance_CSS .= "padding:0 7px;";
							break;
				...
				$caption_CSS = 
						"#jig{$jig_id} .jig-caption-wrapper {
							...
							margin:0 auto;
							padding:".($middle_border_width == 0 ? '0' : '0 '.$middle_border_width.'px').";
							z-index:100;
							overflow:hidden;
				...
				if($link_target == 'off' && !($gallery_type == 'facebook' && $image_element['link_target'] == 'video')){
					...
					if($image_element['link_target'] == 'videoplayer'){
						...
						if ($lightbox !== 'prettyphoto'
							...
							$image_element['link'] = home_url('/').'?'.$this->settings['video_slug'].'='.urlencode($image_element['link']).($this->settings['video_poster'] == 'yes' ? '&poster='.$image_element['url'] : '');
			...
			if($developer_link == 'show'){
				$output .= '<div id="jig'.$jig_id.'-developerLink" class="jig-developerLink"><a href="https://codecanyon.net/item/justified-image-grid-premium-wordpress-gallery/2594251'.($this->settings['envato_user'] != '' ? '?ref='.($this->settings['envato_user'] === "Firsh" ? "Belcovio" : $this->settings['envato_user']) : '').'" target="_blank" title="Justified Image Grid - Premium WordPress Gallery">'.__($developer_link_text,'jig_td').'</a></div>';
			}
		...
		function get_hash_for_external($file_url){
			...
			$cleaned_long_hash = str_replace(array('+', '/', '='), '', $encoded_raw_hash);
			...
			// Image data must exist (but no hash_based_id in it as previously detemined)
			...
			if(property_exists($image_data, 'hash_based_id') === false){
			...
			// Add hash based id to the appropriate row
			...
			if($update_result === false && stripos($wpdb->last_error, "Duplicate entry") !== false){
		...
		function jig_fb_auth() {
			...
			if (empty($_REQUEST["code"])) {
				...
				$dialog_url = "https://www.facebook.com/v3.1/dialog/oauth?client_id=" 
					.$app_id."&scope=user_photos,user_posts,user_videos,pages_show_list"
		...
		function get_fb_user_info($params){
			$graph_url = "https://graph.facebook.com/v3.1/me?fields=name,picture&access_token=" 
					.$params->access_token;
			...
			/* Find user's Albums */
		    $albums_url = 'https://graph.facebook.com/v3.1/me/albums'
		    	.'?fields=id,link,count,from'
		    	.'&limit=1000&access_token='.$params->access_token;
			...
			/* Find user's Pages */
			$pages_url = 'https://graph.facebook.com/v3.1/me/accounts'
		    	.'?access_token='.$params->access_token;
		    	
			$pages = json_decode($this->file_get_contents_curl($pages_url));

			// We are auto-adding the x pages that you manage
			
			if(!empty($pages->data)){
				$output['info']['pages'] = array();
				foreach($pages->data as $page){
					if( !empty($page->perms) && $page->perms[0] !== "ADMINISTER" ||
		...
		function jig_add_fb_page($token = '', $owner = ''){
		...
		function fb_verify_album_data_core($token, $owner, $fb_entity, $type){
			$albums_url = "https://graph.facebook.com/v3.1/"
					.$fb_entity
			...
			if(!empty($albums) && empty($albums['message'])){
				...
				if($found > 0){
					$entity_url = "https://graph.facebook.com/v3.1/".$fb_entity.'?fields=name,picture&access_token='.$token;
					...
					$output = array('error' => __('Cannot use').' '.$albums[0]->from->name.__(', because there are no pictures in any album.', 'jig_td'));
				}
			}else{
				// album data is missing
				$main_url = "https://graph.facebook.com/v3.1/".$fb_entity.'?access_token='.$token;
				$main = json_decode($this->file_get_contents_curl($main_url));
				if($main === false){
					$output = array('error' => __('No albums or SSL certificate problem, verify that the CA cert is OK: <a href="https://snippets.webaware.com.au/howto/stop-turning-off-curlopt_ssl_verifypeer-and-fix-your-php-config/" target="_blank">check this out for more information</a> or go to the General tab, Advanced section and set SSL verify peer setting to No.', 'jig_td'));
				}elseif(!empty($main->error)){
					$output = array('error' => $main->error->message);
				}
			}
			return $output;
		}
		...
		function jig_get_fb_albums(){
			...
			$albums_url = "https://graph.facebook.com/v3.1/".$user_id.'?fields=albums.limit(1000).fields(id,cover_photo,link,count,name,description,type,photos.limit(1).fields(images))&access_token='.$token;
			...
			if(!empty($albums)){
				...
				$videos_url = "https://graph.facebook.com/v3.1/".$user_id.'/videos/uploaded?fields=format.fields(picture)&access_token='.$token;
			...
			if(!empty($this->settings['fb_authed'])){
				...
				foreach($this->settings['fb_authed'] as $key => $val){
					...
					if(empty($val['expires']) || $jig_fb_expires_time > time()+604800){//7776000
						continue;
					}elseif($jig_fb_expires_time > time()){
			...
			if($user != ''){
				if(strrpos($user, 'flickr.com/') === false){
					...
					if(!empty($rsp) && $rsp['stat'] == "ok"){
						...
						$flickr_url = "https://api.flickr.com/services/rest?api_key=".$fli_api_key."&format=php_serial&method=flickr.urls.lookupUser&url=https://www.flickr.com/people/".$user;
						$rsp = maybe_unserialize($this->file_get_contents_curl($flickr_url));
						if(!empty($rsp) && $rsp['stat'] == "ok"){
							$nsid = $rsp['user']['id'];
						}elseif($rsp['code'] == 100){
							$output = array('error' => sprintf(__("The API key is not valid or not set. The Error from Flickr: %s (code %d)", 'jig_td'),'<a href="https://www.webpagefx.com/tools/idgettr/" target="_blank">Flickr user nsid lookup</a>', $rsp['message'],$rsp['code']));
						}elseif(!$rsp){
							$output = array('error' => __('SSL certificate problem, verify that the CA cert is OK: <a href="https://snippets.webaware.com.au/howto/stop-turning-off-curlopt_ssl_verifypeer-and-fix-your-php-config/" target="_blank">check this out for more information</a> or go to the General tab, Advanced section and set SSL verify peer setting to No.', 'jig_td'));
						}else{
							$output = array('error' => sprintf(__("Can't find the user. Try adding the profile URL or your NSID (%s). The error from Flickr: %s (code %d)", 'jig_td'),'<a href="https://www.webpagefx.com/tools/idgettr/" target="_blank">Flickr user nsid lookup</a>', $rsp['message'],$rsp['code']));
						}
					}
		...
		function flickr_collection_walk($user_id, $collection, $fli_api_key, $depth, $flickr_set_list){	
			...
			if(!empty($collection['collection']) && count($collection['collection']) > 0){
				...
				foreach ($collection['set'] as $set) {
					...
					$collection_html .= '<img src="https://farm'.$flickr_set_list[$set['id']]['farm'].'.staticflickr.com/'.$flickr_set_list[$set['id']]['server'].'/'.$flickr_set_list[$set['id']]['primary'].'_'.$flickr_set_list[$set['id']]['secret'].'_s.jpg" />';
		...
		function jig_instagram_search_users(){
			...
			if(!empty($search_result->data)){
				...
				}elseif(count($search_result->data) === 0) {
		...
		function jig_ng_get_recent_images($ng_recent_images, $limit = 0){
			...
			}elseif($ng_recent_images === 'yes_exif'){
		...
		function scrape_youtube_channel($rss_url, $limit, $rss_description){
			...
			$two_col = $yt_initial_data->contents->twoColumnBrowseResultsRenderer;

			if (isset($two_col->tabs[1])) {
				$videos = $two_col
							->tabs[1]
							->tabRenderer
							->content
		...
		function jig_import_settings(){
			...
			$it_is_encrypted = substr($encoded_settings,0,1) === 'y' ? true : false;
